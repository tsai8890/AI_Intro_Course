...
def MST_heuristics(curState, maze, dist_table=None):
    curPos, dotStatus = curState
    pri_queue = PriorityQueue()
    dot_candidates = (*dotStatus, curPos)

    for begin in dot_candidates:
        for end in dot_candidates:
            if begin != end:
                # if dist_table == None or (begin, end) not in dist_table:
                # cost = manhatten_distance(begin, end)
                # else:
                cost = dist_table[(begin, end)]
                pri_queue.push((begin, end, cost), cost)
    
    points = set()
    mst_sum = 0
    while not pri_queue.isEmpty():
        pA, pB, cost = pri_queue.pop()
        if not (pA in points and pB in points):
            points.add(pA)
            points.add(pB)
            mst_sum += cost
        
        if len(points) == len(dot_candidates):
            break
